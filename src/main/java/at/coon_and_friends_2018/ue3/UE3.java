package at.coon_and_friends_2018.ue3;
...
public class UE3 extends BaseMainClass {
	...
	private static void process(InputLinesHolder inputLinesHolder, StringBuilder outputStringBuilder) {
		...
		for (List<Image> images : groupedImages) {
			...
			if (images.get(0).asteroidFootprint.equals("001100001101001011110111111100001100000010")) {
			...
			for (int h = 0; h < currentAsteroidSet.size(); h++) {
				...
				for (int i = h + 1; i < currentAsteroidSet.size(); i++) {
					...
					List<Image> imageGroup = getImageGroup(currentAsteroidSet, firstAsteroid.timestamp, interval,
							observation.startTimestamp, observation.endTimestamp);
...
//				Logger.info("Gefundenes Set");
	...
	private static List<Image> getImageGroup(List<Image> images, int firstTimestamp, int interval, int startobservation,
			int endobservetion) {
		// Logger.info("firstTimestamp " + firstTimestamp + ", interval " +
		// interval);
