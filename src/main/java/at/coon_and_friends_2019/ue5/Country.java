...
public class Country extends BaseObjectClass implements Comparable<Country>{
	...
	public Set<Long> getNeighborCountries() {
		return this.cells.stream().filter(Cell::isBorderCell).map(Cell::getBorderCountries).flatMap(c -> c.stream()).collect(Collectors.toSet());
	}
